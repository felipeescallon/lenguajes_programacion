...
clear
clc
%basado en ejp libro tomasi:ELLOS TOMAN LOS BITS EMPEZANDO DESDE UNO(OJOOOOOOOOOOOOO)
...
RX=input('DE ACUERDO AL MENSAJE CODIFICADO ANTERIOR(t bits), DIGITE(entre corchetes y separados por espacios)\n EL MENSAJE DE RX CON UN ERROR SENCILLO(1 SOLO BIT DE ERROR): RX= ')
disp('MENSAJE DE RX CON 1 ERROR EN UN BIT = ' )
disp(RX)
Errores=abs(TXdef-RX)
Auxil=find(Errores)
...
%¡¡¡¡HA QUEDADO ASEGURADO QUE EL ERROR SERA DE BIT SENCILLO!!!!
%-------------------------------------------------
%-------------------ALGORITMO DE DETECCION DEL BIT DE ERROR----------------------

%-------------------------------------------------
%NO ES ASI!!!!!!!!!!!!!!!:
%PosRx=find(RX)%encuentro las posiciones de los unos
%[row,column]=size(PosRx)

%PONIENDO LOS BITS EN SU NUMERO NORMAL(DE DER A IZQ EMPEZANDO EN CERO) YA QUE MATLAB TOMA LOS INDICES DE IZQ A DER EMPEZANDO EN 1
%g=1
%for contad=1:column
 %   B(g)=t-PosRx(contad)+1%t=n+m
  %  g=g+1
  %end    

%PosUNOSrx=B%VECTOR DE UNOS DEL MSG EN RX como debe ser(der a izq)
%temporal=PosUNOSrx(column)%guardo el primer bit de uno
%error=bitxor(codHam,temporal)

%error=PosUNOSrx(column)%guardo el primer bit de uno
...
%end
%error=dec2bin(error,n)%YA TENGO EL ERROR EN BINARIO(OJO:pero esta como un STRING=>conv a formato numerico
%disp('EL ERROR ES:')
%disp(error) 
%err=str2num(error)%CONVIERTO EL STRING A NUMERO 
%-------------------------------------------------
...
tempo=RX(bitError)%ME DA EL VALOR EN EL BIT DE ERROR:
                    %si es cero(en TXdef era un 1)=>hay que excluir ese UNO de PosUNOSrx
                    %si es uno(en TXdef era un 0)=>hay que incluir ese UNO  a PosUNOSrx  
if (tempo==0) 
    aux=not(TX(bitErrorReal))
...
for j=1:column
    error=bitxor(error,PosUNOSrx(column))
end
...
%TAMBIEN SE ASUMIRA UN ERROR CONOCIDO(POSICION ESPECIFICA):SO FAR; THE IDEAS IS TO MAKE IT WORK WITH ANY ERROR POSITION!!! 
%ENTONCES TENEMOS:
