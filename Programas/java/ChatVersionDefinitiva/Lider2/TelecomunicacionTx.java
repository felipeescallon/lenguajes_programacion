...
String CadenaTotal;									 //y una variable que indica la funcion	
...
public void VariosaUno(TextField VariosaUno)						  //METODO UTILIZADO PARA SATISFACER EL ENVIO DE TIPO (VARIOS A UNO)
{											  //Metodo utilizado por la interfaz de lider y encargado de enviar el mensaje a los
											  //integrantes del grupo opuesto al que el lider pertenece		
	try
	{
	Flujohaciaservidor.writeInt(-6);						  //(i=-6): SE LE INDICA AL SERVIDOR QUE EL MENSAJE DEBE SER ENVIADO A LOS INTEGRANTES DEL GRUPO OPUESTO AL QUE EL REMITENTE PERTENECE  	
	Flujohaciaservidor.writeUTF(CampoLogin);					  //Enviamos el login del usuario que es lider de un grupo
	Flujohaciaservidor.writeUTF(VariosaUno.getText());				  //Enviamos el mensaje que llegara a los usuarios pertenecientes al otro grupo	
	}
